...
class BannerHttpClientTests(unittest.IsolatedAsyncioTestCase):
    async def test_probe_grades_session_returns_false_when_storage_state_is_missing(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            settings = _settings(temp_path)
            client = BannerHttpClient(
                settings, SessionStateStore(settings.storage.storage_state_path)
            )

            self.assertFalse(await client.probe_grades_session())

    async def test_probe_grades_session_returns_true_for_authenticated_grades_page(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
    ...
    async def test_probe_grades_session_returns_false_after_redirect_to_login(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            ...
            def handler(request: httpx.Request) -> httpx.Response:
                ...
                return httpx.Response(
                    200, text="<html><body>Sign in</body></html>", request=request
                )
    ...
    async def test_get_json_uses_relative_paths_and_loaded_cookies(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def handler(request: httpx.Request) -> httpx.Response:
                self.assertEqual(
                    request.url.path, "/StudentSelfService/studentGrades/term"
                )
            ...
            payload = await client.get_json(
                "/StudentSelfService/studentGrades/term", params={"page": 1}
            )
