...
(() => {
  const button = document.getElementById("fetchAttendanceButton");
  const feedback = document.getElementById("attendanceFeedback");
  const message = document.getElementById("attendanceMessage");

  if (!button || !feedback || !message) {
    return;
  }

  const resultKey = "uaAttendanceFetchResult";
  const cooldownKey = "uaAttendanceFetchCooldownUntil";
  const defaultLabel = "Actualizar asistencia";
  let cooldownTimer = null;
  let isRunning = false;
  ...
  async function readError(response) {
    ...
    return { message: "No se pudo actualizar asistencia." };
  }
  ...
  async function refreshStatus() {
    ...
    try {
      const response = await fetch("/api/attendance/fetch/status");
      if (!response.ok) {
        return;
      }
      const status = await response.json();
      if (status.running) {
        button.disabled = true;
        button.textContent = "Actualizando...";
        setFeedback("Ya hay una actualizacion de asistencia en curso.", "loading");
        window.setTimeout(refreshStatus, 3000);
        return;
      }
      if (status.cooldown_seconds > 0) {
  ...
  async function fetchAttendance() {
    ...
    button.disabled = true;
    button.textContent = "Actualizando...";
    setFeedback("Actualizando asistencia desde Banner...", "loading");

    try {
      const response = await fetch("/api/attendance/fetch", { method: "POST" });
      if (!response.ok) {
        const error = await readError(response);
        if (error.cooldown_seconds > 0) {
          setFeedback(error.message || "Espera antes de volver a actualizar.", "loading");
          ...
          setFeedback(error.message || "No se pudo actualizar asistencia.", "error");
          button.disabled = false;
          button.textContent = defaultLabel;
        }
        return;
      ...
      const result = await response.json();
      setFeedback(result.message || "Asistencia actualizada.", "success");
      startCooldown(result.cooldown_seconds || 60);
      sessionStorage.setItem(resultKey, JSON.stringify(result));
      window.setTimeout(() => window.location.reload(), 900);
    } catch (error) {
  ...
  if (savedResult) {
    ...
    try {
      ...
      setFeedback("Asistencia actualizada.", "success");
    }
  }
