...
def _build_parser() -> argparse.ArgumentParser:
    ...
    fetch_parser = subparsers.add_parser("fetch", help="Extrae el historial desde Banner")
    fetch_parser.add_argument("--full", action="store_true", help="Recarga todos los semestres en lugar de solo el actual")
...
async def _run_fetch(*, full_refresh: bool) -> None:
    ...
    contract_capture = BannerHttpContractCapture(debug_store) if settings.browser.capture_banner_contract else None
    http_client = BannerHttpClient(settings, session_store, contract_capture=contract_capture)
    ...
    gateway = BannerGateway(settings=settings, debug_store=debug_store, http_client=http_client)
    ...
    try:
        history = await use_case.execute(previous_history=previous_history, full_refresh=full_refresh)
