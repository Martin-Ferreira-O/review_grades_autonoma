...
class _BannerContractCaptureBase:
    _TARGET_PATHS = {
    ...
    def _summary_path(self, debug_store: DebugArtifactStore, captures: list[dict[str, Any]], errors: list[dict[str, str]]) -> str:
        summary = {
...
class BannerContractCapture(_BannerContractCaptureBase):
    ...
    async def stop(self) -> str | None:
        ...
        return self._summary_path(self._debug_store, self._captures, self._errors)
        ...
        try:
            ...
            capture = {
                ...
                "response": {
                    ...
                    "body": self._parse_payload(await response.text()),
                },
            }
            ...
            self._captures.append({"sequence": sequence, "path": path, "file": filename})
        except Exception as error:
            self._errors.append({"sequence": str(sequence), "path": path, "error": str(error)})


class BannerHttpContractCapture(_BannerContractCaptureBase):
    def __init__(self, debug_store: DebugArtifactStore):
        self._debug_store = debug_store
        self._captures: list[dict[str, Any]] = []
        self._errors: list[dict[str, str]] = []
        self._sequence = 0
    ...
    async def record(self, response: httpx.Response) -> None:
        ...
        self._sequence += 1
        try:
            request_content = response.request.content.decode("utf-8") if response.request.content else None
            capture = {
                ...
                "request": {
                    ...
                    "path": path,
                    "query": self._normalize_query(str(response.request.url)),
                    "headers": self._filter_headers(dict(response.request.headers), self._REQUEST_HEADER_ALLOWLIST),
                    "post_data": self._parse_payload(request_content),
                },
                "response": {
                    "status": response.status_code,
                    "headers": self._filter_headers(dict(response.headers), self._RESPONSE_HEADER_ALLOWLIST),
                    "body": self._parse_payload(response.text),
                },
            }
            filename = f"banner_contract/{self._sequence:02d}_{self._path_key(path)}.json"
            self._debug_store.write_text(filename, json.dumps(capture, indent=2, ensure_ascii=True))
            self._captures.append({"sequence": self._sequence, "path": path, "file": filename})
        except Exception as error:
            self._errors.append({"sequence": str(self._sequence), "path": path, "error": str(error)})
